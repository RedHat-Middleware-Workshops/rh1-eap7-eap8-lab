...
Also in the pom.xml file add the following before the closing tag replacing the existing <profiles> block:
...
To deploy our application to OpenShift we need to push our code changes to our gitea repository.
...
The "eap8-build-artifacts" will start automatcally.  This build is responsible for deploying the required instance of JBoss EAP, based on the feature packs and layers configured by the eap-maven-pluggin (as described above), configuring JBoss EAP (e.g. to connect to a database), and building and deploying the application.
...
It will take a while (around 8 minutes) to build and deploy the application. To monitor the progress, follow these steps.
